...
// These methods are called right after corresponding editor script's
...
int AppEditorLogic::update() {
  // Write here code to be called before updating each render frame when editor
  // is loaded.
  return 1;
}
...
int AppEditorLogic::render(const Unigine::EngineWindowViewportPtr &window) {
  ...
  // Write here code to be called before rendering each render frame when editor
  // is loaded.
  return 1;
}
